...
use App\Filament\Resources\IzinSeleksiResource\Pages;
use pxlrbt\FilamentExcel\Actions\Tables\ExportBulkAction;
use Livewire\Features\SupportFileUploads\TemporaryUploadedFile;
use App\Filament\Resources\IzinSeleksiResource\RelationManagers;
                                                ...
                                                Section::make('Data Kepegawaian')
                                                    ...
                                                    ->schema([
                                                        ...
                                                        FileUpload::make('dokumen_pengembangan_kompetensi')
                                                            ->directory('lampiran_persyaratan_pengajuan_tugas_belajar')
                                                            ->label('Dokumen Lainnya (Opsional)')
                                                            ->panelLayout('compact')
                                                            ->acceptedFileTypes([
                                                                'application/pdf',
                                                                'image/*',
        ...
        return $table
            ...
            ->actions([
                ...
                ActionGroup::make([
                    ...
                    EditAction::make()->visible(fn(TugasBelajar $record): bool => ($record->status === 'pending' || $record->status === 'rejected') && ($record->stage === 'tahap_opd' || $record->stage === 'tahap_bkpsdm')),
                    Action::make('Cetak Surat Permohonan')
                        ->label('Cetak Surat Permohonan')
                        ->action(
                            function (TugasBelajar $record) {
                                Notification::make()
                                    ->title('Surat Didownload: Mohol Upload Surat Yang Telah Didownload dengan yang sudah di ttd basah')
                                    ->success()
                                    ->send();
